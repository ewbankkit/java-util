...
import static com.netsol.adagent.util.beans.BaseData.any;
import static com.netsol.adagent.util.beans.BaseData.arrayContains;
...
public final class FeatureId {
    ...
    public static boolean anyFeatureIsEnabled(final int[] featureIds, int... features) {
        if (arrayIsNotEmpty(features)) {
            for (int i = 0; i < features.length; i++) {
                if (arrayContains(featureIds, features[i])) {
                    return true;
                }
            }
        }
    ...
    public static boolean anyFeatureIsEnabled(final Collection<Integer> featureIds, int... features) {
        if (collectionIsEmpty(featureIds)) {
            return false;
        }
        return any(toIterable(features), new Predicate<Integer>() {
            @Override
